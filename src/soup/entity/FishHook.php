...
namespace soup\entity;

use Ghezin\cp\items\FishingRod;
...
class FishHook extends Projectile{
	...
	public function __construct(Level $level, CompoundTag $nbt, ?Entity $owner=null){
		...
		if($owner instanceof Player){
			...
			Main::getInstance()->startFishing("b", $owner);
	...
	public function entityBaseTick(int $tickDiff=1):bool{
		...
		$owner=$this->getOwningEntity();
		if($owner instanceof Player){
			if(!$owner->getInventory()->getItemInHand() instanceof \soup\items\FishingRod or !$owner->isAlive() or $owner->isClosed())
				$this->flagForDespawn();
	...
	public function close():void{
		...
		$owner=$this->getOwningEntity();
		if($owner instanceof Player){
			Main::getInstance()->stopFishing($owner);
		}
	}
